...
#pragma omp parallel for schedule(dynamic) default(none) private(x, y, pc, i, sum, offset, channel) shared(gv, b, ext, a, index, zeros, dimYMinus1, dimX, dimXMinus1, dimY, numChannels)
        for (y = 0; y < a.dimY; y++) {
            for (x = 0; x < a.dimX; x++) {
...
#pragma omp parallel for schedule(dynamic) default(none) private(x, y, pc, i, sum, offset, channel) shared(gv, b, ext, a, index, zeros, dimY, dimXMinus1, dimX, dimYMinus1, numChannels)
    for (x = 0; x < a.dimX; ++x) {
        for (y = 0; y < a.dimY; ++y) {
            pc = get_pixel(b, x, y);
            int deta = min(min(a.dimY - y - 1, y), min(a.dimX - x - 1, x));
