...
ray generate_ray(const ray *camera, unsigned short xi[3],
    ...
    vec3 camera_ray_dir = vec3_unit(vec3_add(vec3_mulf(*cx, (((sx + .5 + dx) / 2 + x) / image_width - .5)),
                                             vec3_add(vec3_mulf(*cy, (((sy + .5 + dy) / 2 + y) / image_height - .5)),
                                                      camera->d)));
    /* Shoot the ray from camera */
    return ray_init(vec3_add(camera->o, vec3_mulf(camera_ray_dir, camera_offset)),
                    camera_ray_dir); /* Offset = 140 is added to place the ray deep into the scene because the box-like walls are actually made of large spheres */
}
...
vec3 radiance(const ray r, int depth, unsigned short *xi) {
    ...
    if (obj->material == DIFFUSE) { /* Ideal diffuse reflection */
        ...
        vec3 d = vec3_unit(vec3_add(vec3_mulf(u, cos(r1) * r2s),
                                    vec3_add(vec3_mulf(v, sin(r1) * r2s), vec3_mulf(w, sqrt(1 - r2)))));
        ...
        return vec3_add(obj->emission, vec3_mul(f, radiance(
                ray_init(x, vec3_sub(r.d, vec3_mulf(n, (2 * vec3_dot(n, r.d))))), depth, xi)));
        ...
        return vec3_add(obj->emission, vec3_mul(f, (depth > 2 ? (erand48(xi) < P ? /* Russian roulette */
                                                                 vec3_mulf(radiance(r_reflect, depth, xi), RP)
                                                                                 : vec3_mulf(
                        radiance(r_transmit, depth, xi), TP)) :
                                                    vec3_add(vec3_mulf(radiance(r_reflect, depth, xi), Re),
                                                             vec3_mulf(radiance(r_transmit, depth, xi), Tr)))));
    } else
        return vec3_init(0, 0, 0);
}
...
void *
ray_tracing(void *paras) {
    int i;
    int y, x, sy, sx, s;
    ray_tracing_paras *tmp = (ray_tracing_paras *)paras;
    for (y = tmp->y_beg; y < tmp->y_end; ++y) {
        unsigned short xi[3] = {0, 0};
        xi[2] = y * y * y;
        for (x = 0; x < tmp->image_width; ++x) {
            i = (tmp->image_height - y - 1) * tmp->image_width + x;
            for (sy = 0; sy < 2; ++sy) {
                for (sx = 0; sx < 2; ++sx) {
                    vec3 accumulated_color = vec3_init(0, 0, 0);
                    for (s = 0; s < tmp->samples_per_pixel; s++) {
                        /* Randomly generate a ray from camera */
                        ray camera_ray = generate_ray(&tmp->camera, xi, tmp->image_width, tmp->image_height,
                                                      x, y, &tmp->cx, &tmp->cy, sx, sy);
                        ...
                        accumulated_color = vec3_add(accumulated_color,
                                                     vec3_mulf(camera_ray_color, (1. / tmp->samples_per_pixel)));
                    }
                    /* Add `accumulated_color` for this subpixel to the color for the image pixel, which can be imagined as 1 = 0.25 * 4 */
                    tmp->image[i] = vec3_add(tmp->image[i], vec3_mulf(
                            vec3_init(clamp(accumulated_color.x), clamp(accumulated_color.y),
                                      clamp(accumulated_color.z)), .25));
                }
            }
        }
    }
...
/*    int i;
    ...
    for (j = 0; j < 4; ++j) {
        paras[j].image = image;
        paras[j].image_width = image_width;
        paras[j].image_height = image_height;
        paras[j].camera = camera;
        paras[j].samples_per_pixel = samples_per_pixel;
        paras[j].cx = cx;
        paras[j].cy = cy;
        paras[j].y_beg = j * unit;
        paras[j].y_end = (j + 1) * unit;
        if (j == 3) {
            paras[j].y_end = image_height;
        }
    }
    for (j = 0; j < 4; ++j) {
        pthread_create(&threads[j], NULL, ray_tracing, &(paras[j]));
    }
    for (j = 0; j < 4; ++j) {
            ...
            for (sy = 0; sy < 2; ++sy) {
                for (sx = 0; sx < 2; ++sx) {
                    ...
                    for (s = 0; s < samples_per_pixel; s++) {
                        ...
                        *//* Add the color to `accumulated_color` for this subpixel *//*
                        accumulated_color = vec3_add(accumulated_color,
                                                     vec3_mulf(camera_ray_color, (1. / samples_per_pixel)));
                    }
                    *//* Add `accumulated_color` for this subpixel to the color for the image pixel, which can be imagined as 1 = 0.25 * 4 *//*
                    image[i] = vec3_add(image[i], vec3_mulf(
                            vec3_init(clamp(accumulated_color.x), clamp(accumulated_color.y),
                                      clamp(accumulated_color.z)), .25));
                }
            }
